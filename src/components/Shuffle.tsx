...
export default function Shuffle({
  ...
  useGSAP(
    () => {
      ...
      const build = () => {
        ...
        chars.forEach((ch: HTMLElement) => {
          ...
          if (shuffleDirection === 'right') {
            startX = -steps(w, rolls);
            finalX = 0;
          } else if (shuffleDirection === 'left') {
            startX = 0;
            finalX = -steps(w, rolls);
          } else if (shuffleDirection === 'down') {
            startY = -steps(h, rolls);
            finalY = 0;
          } else if (shuffleDirection === 'up') {
            startY = 0;
            finalY = -steps(h, rolls);
          }
      ...
      const steps = (size: number, rolls: number) => (rolls + 1) * size;

      const inners = () => wrappersRef.current.map(w => w.firstElementChild).filter((el): el is Element => el !== null);
  ...
  const commonStyle = useMemo(() => ({ textAlign, ...style } as CSSProperties), [textAlign, style]);
